...
Let $E/\F_q : y^2 = x^3+ax+b$ be the public curve of the cryptosystem.
...
This is mainly due to $b$ not beeing used for scalar multiplication on $E$.

\begin{enumerate}
\item Mallory may send an honest $P = (x,y)$ point on $E$ and gets back some \verb|k|$.P$ from Bob.
But Mallory may also send a malicious point $Q = (\tilde{x},\tilde{y})$ on another curve $\tilde{E} : y^2 = x^3+ax+\tilde{b}$ (note that the $a$ coefficient remains unchanged).
Since formulae for computing \verb|k|$.Q$ do not involve $b, \tilde{b}$, Bob will correctly compute \verb|k|$.Q$, believing he just made a computation on $E$.
...
\item apport de montgomery qui limite le choix de E au twist (according to ...)
\end{itemize}
