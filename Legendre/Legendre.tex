...
Il est bien connu (voir \cite{Silverman:EC}, Theorem V.4.1) que $E_{\lambda}$ est supersingulière si et seulement si $\lambda$ est racine de
\[
...
En effet, $H_p(\lambda)$ est intimement lié à la trace du Frobenius de $E_{\lambda}/\F_p$ d'une part et $|E_{\lambda}(\F_p)| \in 4 \Z$ d'autre part (voir \cite{Auer-Top}).
...
\begin{itemize}
  ...
  \item $f(x) \in (\F_q^\times)^2 \iff y^2 = f(x)$ a deux  solutions distinctes sur $\F_q$.
\end{itemize}
...
Donc $\left|(\F_q^\times)^2\right| = \frac{q-1}{2}$ et $(\F_q^\times)^2$ est l'unique sous-groupe d'ordre $\frac{q-1}{2}$ de  $\F_q^\times = \langle \alpha \rangle$, i.e. $(\F_q^\times)^2 = \langle \alpha^2 \rangle$.

\begin{proposition}
$\forall  y \in \F_q, \; \chi(y) = y^{\frac{q-1}{2}}$ en tant qu'égalité dans $\F_q$.
\end{proposition}

\begin{preuve}
Tout d'abord, $\chi(0) = 0 = 0^{\frac{q-1}{2}}$.

\noindent Soit $y \in \F_q^{\times}, \; \chi(y)=1 \iff y \in (\F_q^\times)^2
\iff y = (\alpha^2)^i,\ i \in \mathbb{Z}
\iff y^{\frac{q-1}{2}} = 1$.  
Détaillons cette dernière équivalence.
Soit $y \in \F_q^{\times}$ tel que $ y = (\alpha^2)^i$, alors $y^{\frac{q-1}{2}} = \alpha^{(q-1)i} = 1$ car $\langle \alpha \rangle = \F_q^\times.$
Réciproquement,
\[
y^{\frac{q-1}{2}} = 1
\;\Rightarrow\;
...
\noindent Puisque $\F_q = \{x \in \overline{\F_p} \mid x^q - x = 0\}$,  $\F_q^\times = \{ x \in \F_q \mid x^{q-1} - 1 =0\rbrace$, le polynôme $X^{q-1}-1$ a pour fonctions symétriques élémentaires en ses racines $\sigma_1 = \cdots = \sigma_{q-2} = 0, \sigma_{q-1} = (-1)^{q}.$

\noindent Ici, $p_1= p_2 = \dots= p_{q-2}=0$ 
...
et $ p_{q-1}= \sum_{x \in \F_q} x^{q-1} = 0^{\,q-1} + \sum_{x \in \F_q^\times} x^{q-1} = 0 + \sum_{x \in \F_q^\times} 1= q-1 = -1.$ 
\[\tag*{$\square$}\]
\end{preuve}
\vspace*{.6cm}


\noindent Donc dans la relation $(\dagger)$ seuls les monômes de $f(x)^{\frac{q-1}{2}}$ de degré un multiple de $q-1$ contribuent à la somme.
Or \(\deg f(x)=3\), donc
\[
\deg f(x)^{\frac{q-1}{2}} = \frac{3}{2}(q-1)
\]
et les seuls multiples entiers de $q-1$ dans $[ 0;\frac32(q-1)]$ sont $0$ et $q-1$.
Si on note $\alpha$ le coefficient constant de $f(x)^{\frac{q-1}{2}}$ alors $\sum_{x \in \F_q} \alpha = q\alpha = 0$ donc seul le monôme de degré $q-1$ contribue dans la somme $(\dagger)$. 
...
\noindent D’après le théorème d'isogénie de Tate,
\[
...
\begin{enumerate}
\item ce n’est une description que sur $\overline{\F}_p$,
...
Nous avons cependant besoin de comprendre un certain nombre de relations provenant d’isomorphismes sur $\F_p$.
Nous collectons dans ce paragraphe des résultats sur ce sujet.
À partir de maintenant, $E_1 \simeq E_2$ signifiera un isomorphisme $\F_p$-rationnel de courbes elliptiques $E_1/\F_p$, $E_2/\F_p$.
De même, $E_1 \sim E_2$ signifiera une isogénie $\F_p$-rationnelle de courbes elliptiques $E_1/\F_p$, $E_2/\F_p$.
La courbe
\[
...
a un modèle $y^2 = (x-a)(x-b)(x-c), \; a,b,c \in \F_p$
et l’automorphisme
\[
\begin{cases}
x \mapsto x+a \\
...
\end{cases}
\]
ramène au modèle $E : y^2 = x(x-\alpha)(x-\beta), \; \alpha,\beta \in \F_p$.
On traite donc dans cet énoncé des courbes elliptiques sur $\F_p$ ayant leur $2$-torsion rationnelle.
On étudie les isomorphismes de courbes elliptiques suivants
...
\noindent L'expression d'un isomorphisme de courbes elliptiques sous forme de Weiestrass est bien connue.
...
\begin{preuve}
\cite{Auer-Top} renvoie à \cite{Silverman:EC} \S X.1 qui traite de la $2$-descente (dans le cas des corps de nombres donc) ou à \cite{Schaefer} qui n’énonce un résultat que pour les courbes hyperelliptiques.
...
\begin{definition}
Soit $E/k : y^2 = f(x)$ une courbe elliptique.
Soit $\alpha \in k^\times$ alors
\[
E^{(\alpha)} : \alpha y^2 = f(x)
\]
défini une courbe elliptique appelée \emph{twist quadratique} de $E$ par $\alpha$.
\end{definition}

\begin{remark}
\begin{enumerate}
\item  Pour $\alpha \neq 1, \; \alpha y^2 = f(x) = x^3 + a x^2 + b x + c
\simeq
...
\item Le twist est trivial si et seulement si on a un isomorphisme $k$-rationnel de courbes elliptiques $E \simeq_k E^{(\alpha)}$.
\item Soit $E/\F_q$ et $\alpha \notin (\F_q^\times)^2$ alors $|E(\F_q)| + |E^{(\alpha)}(\F_q)| = q + 2$.
\end{enumerate}
\end{remark}

\begin{proposition}\label{j1728}
Soit $E/k$ une courbe elliptique et $\alpha \in k^{\times} \setminus (k^\times)^2$.
\[
...
\noindent Si $E \simeq E^{(\alpha)}$, alors $E^{(\alpha)}$ a pour modèle $ y^2 = x^3 + \alpha^2 A x + \alpha^3 B$.
Un isomorphisme $\varphi : E \longrightarrow E^{(\alpha)}$ est de la forme $\varphi(x,y) = (u^2 x, u^3 y), \; u \in  (k^{\times})$ et vérifie
\[
\begin{cases}
...
Donc si $-1,\ \lambda,\ 1-\lambda \in (\F_q^\times)^2$
\[
...
E_{1-1/\lambda}.
\]
\noindent Enfin, $\ 1-\lambda \in (\F_q^\times)^2 \Rightarrow \;  E_{1-\lambda}  \simeq E_{1-\lambda}^{(1-\lambda)} \simeq E_{1/(1-\lambda)}$.
\item[(b) $\Rightarrow$ (c)]
On a
\[
...
D'après la Proposition \ref{2descente}, $(0,0),\ (1,0),\ (\lambda,0) \in 2E_\lambda(\F_q)$.
Par exemple,
\[
(0,0) = 2P_0,
\qquad
...
De même avec les points
\[
(1,0) = 2P_1, \; (\lambda,0) = 2P_\lambda, \qquad P_1, P_\lambda \in E_\lambda[4](\F_q).
\]

Donc $|E_\lambda[4](\F_q)| > 10$ et $E_\lambda[4](\F_q)$ est un sous-groupe de $E_\lambda[4](\overline{\F}_q) \simeq \mathbb{Z}/4\mathbb{Z} \times \mathbb{Z}/4\mathbb{Z}$.
Par conséquent
\[
E_\lambda[4](\F_q)
\simeq
...
\noindent Soit $P \in E_{\lambda}$ tel que $2P = \mathcal{O}$, et $Q$ tel que $2Q = P$, en particulier $Q \in E_\lambda[4](\overline{\F}_q) = E_\lambda[4](\F_q)$ i.e. toute la \(2\)-torsion est dans \(2E_\lambda(\F_q)\).
...
En particulier, comme $\sigma(Q) \in  E[2](\F_{p^n})$
\[
\sigma(Q) = Q + 2P,
\]
en effet si \( \sigma(Q) = Q \), alors \( Q \in E(\F_p) \), et si \( \sigma(Q) = 2P \), alors \( Q = \sigma^{-1}(2P) \in E(\F_p) \).
D’où
\begin{align*}
...
\, a,b,c \in \F_p $ puis  appliquer l’isomorphisme $(x,y) \longmapsto (x+a,\, y)$ pour obtenir le modèle
\[
E/\F_p : \quad
...
\begin{align*}
E^{(\alpha)} : \quad
y^2 = x^3 - \alpha(\alpha+\beta)x^2 + \alpha^3\beta x \;&\simeq\; E_{\beta/\alpha} :\;
...
Si \(p = 3\), alors \(k \in \{-1,0,1\}\), cependant
\[
\begin{cases}
...
\begin{enumerate}
\item[(a)] Si \(E_\lambda^{(\alpha)}\) est Legendre isogène, c'-à-d $E_\lambda^{(\alpha)} \sim E_\mu, \, \mu \in \F_p$.
En notant \(\operatorname{Tr}_{\F_p}(E)\) la trace du Frobenius de \(E/\F_p\)
\[
...
Donc, d'après le théorème de Tate $ E \sim E_\lambda^{(\alpha)}$.
D'où $E \sim E_\lambda^{(\alpha)} \sim E_\mu$ i.e.  \(E\) est Legendre isogène sur \(\F_p\).
...
\[
E'[2^{\alpha-1}](\F_p) \simeq \mathbb{Z}/2\mathbb{Z} \times \mathbb{Z}/2^{\alpha-1}\mathbb{Z}.
\]
Donc
\[
E'[4](\F_p)
...
Quitte à effectuer $(x,y) \mapsto (x - x_Q, y)$, on peut supposer qu'on a un modèle  \(E' : y^2 = x(x-\alpha')(x-\beta'), \; \alpha', \beta' \in \F_p\),  avec $(0,0) \in 2E'(\F_p)$.
...
\]
En particulier, $E_{\lambda'}^{(\alpha)}$ est Legendre isomorphe, i.e $E_{\lambda'}^{(\alpha)} \simeq E_\mu$.
Enfin, on récapitule 
\[
...
La borne de Hasse donne $|t|< 2\sqrt{p}$.

\begin{align*}
|t|< 2\sqrt{p}
\iff & t^2 < 4p
\iff (p+1+4k)^2 < (2\sqrt{p})^2\\
\iff &(p+1+4k+2\sqrt{p})(p+1+4k-2\sqrt{p}) < 0\\
\iff &(4k+(\sqrt{p}+1)^2)(4k+(\sqrt{p}-1)^2)< 0\\
...
\]
Tous les multiples de $4$ de cet intervalle apparaissent comme ordre d'une courbe sous forme de Legendre sur $\F_p$.
\item De plus les valeurs apparaissant dans $\bigl(H_p(\lambda) \bmod p \,;\, \lambda \in \F_p^2\setminus\{0,1\}\bigr)$ sont soumises aux conditions
\[ m \textrm{ est pair }
...
\item On remarque les symétries suivantes 
\[
...
\noindent Revenons à la preuve de la proposition \ref{2descente}.
Soient $m=2$, $p \ge 3$ premier et
\[
E/\mathbb{F}_p : y^2 = (x-\alpha)(x-\beta)(x-\gamma), \quad \alpha,\beta,\gamma \in \mathbb{F}_p.
\]
Alors $f_{P_\alpha} = x-\alpha \in \mathbb{F}_p(E)$ a pour diviseur $\mathrm{div}(f_{P_\alpha}) = 2(P_\alpha) - 2(\mathcal{O})$, où $P_\alpha = (\alpha,0) \in E(\mathbb{F}_p)$.
De plus, en notant
\[
\begin{cases}
